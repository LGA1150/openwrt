...
sf_ce_ahash_cra_init(struct crypto_tfm *tfm)
{
	...
	crypto_ahash_set_reqsize(__crypto_ahash_cast(tfm),
				 sizeof(struct sf_ce_ahash512_reqctx));
	ctx->priv = g_dev;
...
static int
sf_ce_sha512_init(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct device *dev = ctx->priv->dev;
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	__be64 *hashw = (__be64 *)reqctx->hash;
	dma_addr_t hash_phys, block_phys;
	int ret;

	memset(reqctx, 0, sizeof(*reqctx));
	hashw[0] = cpu_to_be64(SHA512_H0);
	hashw[1] = cpu_to_be64(SHA512_H1);
	hashw[2] = cpu_to_be64(SHA512_H2);
	hashw[3] = cpu_to_be64(SHA512_H3);
	hashw[4] = cpu_to_be64(SHA512_H4);
	hashw[5] = cpu_to_be64(SHA512_H5);
	hashw[6] = cpu_to_be64(SHA512_H6);
	hashw[7] = cpu_to_be64(SHA512_H7);
...
static int
sf_ce_sha384_init(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct device *dev = ctx->priv->dev;
	...
	memset(reqctx, 0, sizeof(*reqctx));
	hashw[0] = cpu_to_be64(SHA384_H0);
	hashw[1] = cpu_to_be64(SHA384_H1);
	hashw[2] = cpu_to_be64(SHA384_H2);
	hashw[3] = cpu_to_be64(SHA384_H3);
	hashw[4] = cpu_to_be64(SHA384_H4);
	hashw[5] = cpu_to_be64(SHA384_H5);
	hashw[6] = cpu_to_be64(SHA384_H6);
	hashw[7] = cpu_to_be64(SHA384_H7);
...
static int
sf_ce_sha512_update(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	 ...
	 */
	partial_offset = reqctx->length & (SHA512_BLOCK_SIZE - 1);
	if (partial_offset + req->nbytes < SHA512_BLOCK_SIZE) {
	...
	pktlen = ALIGN_DOWN(partial_offset + req->nbytes, SHA512_BLOCK_SIZE);
	/* new_partial_offset: new partial data offset after update */
	new_partial_offset = (reqctx->length + req->nbytes) & (SHA512_BLOCK_SIZE - 1);
	...
	tx[cur_tx].des2 = cpu_to_le32(FIELD_PREP(CE_TDES2_ED, CE_ENDIAN_BIG) | FIELD_PREP(CE_TDES2_B1L, SHA512_DIGEST_SIZE)|
	...
	tx[cur_tx].des3 = cpu_to_le32(CE_TDES3_OWN | CE_TDES3_FD | FIELD_PREP(CE_TDES3_B1T, CE_BUF_HASH_INIT) |
				      FIELD_PREP(CE_TDES3_CT, CE_SHA512) | FIELD_PREP(CE_TDES3_PL, pktlen));
	pr_debug("tdes0: %08X\n", tx[cur_tx].des0);
	pr_debug("tdes1: %08X\n", tx[cur_tx].des1);
	pr_debug("tdes2: %08X\n", tx[cur_tx].des2);
	pr_debug("tdes3: %08X\n", tx[cur_tx].des3);
	...
	for_each_sg(req->src, sg, nents, i) {
		...
		if (i % 2 == 0) { /* buffer at des0 */
			...
			tx[cur_tx].des3 = cpu_to_le32(CE_TDES3_OWN | FIELD_PREP(CE_TDES3_LD, ld) |
						      ...
						      FIELD_PREP(CE_TDES3_CT, CE_SHA512) |
		...
		print_hex_dump(KERN_INFO, "src ", DUMP_PREFIX_OFFSET, 16, 1,
	...
	rx[cur_rx].des2 = cpu_to_le32(FIELD_PREP(CE_RDES2_B1L, SHA512_DIGEST_SIZE));
...
static int
sf_ce_sha512_final(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	...
	/* Append length */
	put_unaligned_be64((reqctx->length * BITS_PER_BYTE) >> 64, reqctx->block + index + pad_len);
	put_unaligned_be64(reqctx->length * BITS_PER_BYTE, reqctx->block + index + pad_len + sizeof(u64));
	...
	tx[cur_tx].des3 = cpu_to_le32(CE_TDES3_OWN | CE_TDES3_FD | CE_TDES3_LD |
				      ...
				      FIELD_PREP(CE_TDES3_CT, CE_SHA512) | FIELD_PREP(CE_TDES3_PL, index + pad_len));
	pr_debug("tdes0: %08X\n", tx[cur_tx].des0);
	pr_debug("tdes1: %08X\n", tx[cur_tx].des1);
	pr_debug("tdes2: %08X\n", tx[cur_tx].des2);
	pr_debug("tdes3: %08X\n", tx[cur_tx].des3);
	...
	print_hex_dump(KERN_INFO, "reqctx->block ", DUMP_PREFIX_OFFSET, 16, 1,
		       reqctx->block, reqctx->block_len, true);
	reqctx->nents = 0;
	reqctx->final = true;
	/* inform the DMA for the new data */
	dma_wmb();
...
static int
sf_ce_sha512_finup(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	...
	/* Pad out to 112 mod 128. */
	index = (req->nbytes + partial_offset + reqctx->length) & 0x7f;
	pad_len = (index < 112) ? (112 - index) : ((128+112) - index);
	...
	/* Append length */
	put_unaligned_be64(((req->nbytes + reqctx->length) * BITS_PER_BYTE) >> 64,
			   reqctx->block + partial_offset + pad_len);
	put_unaligned_be64((req->nbytes + reqctx->length) * BITS_PER_BYTE,
			   reqctx->block + partial_offset + pad_len + sizeof(u64));
...
static int
sf_ce_sha512_digest(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct sf_ce_ahash_ctx *ctx = crypto_ahash_ctx(tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
...
static int
sf_ce_sha512_export(struct ahash_request *req, void *out)
{
	struct sf_ce_ahash_ctx *ctx = crypto_tfm_ctx(req->base.tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	struct sf_ce_sha512_state *state = out;
	struct device *dev = ctx->priv->dev;
...
static int
sf_ce_sha512_import(struct ahash_request *req, const void *in)
{
	struct sf_ce_ahash_ctx *ctx = crypto_tfm_ctx(req->base.tfm);
	struct sf_ce_ahash512_reqctx *reqctx = ahash_request_ctx(req);
	const struct sf_ce_sha512_state *state = in;
	struct device *dev = ctx->priv->dev;
	dma_addr_t hash_phys, block_phys;
	int ret;
...
struct ahash_alg sf_ce_sha512 __read_mostly = {
	.init	= sf_ce_sha512_init,
	.update	= sf_ce_sha512_update,
	.final	= sf_ce_sha512_final,
	.finup	= sf_ce_sha512_finup,
	.digest	= sf_ce_sha512_digest,
	.export = sf_ce_sha512_export,
	.import = sf_ce_sha512_import,
	.halg.digestsize	= SHA512_DIGEST_SIZE,
	.halg.statesize		= sizeof(struct sf_ce_sha512_state),
	.halg.base	=	{
		.cra_name		= "sha512",
		.cra_driver_name	= "siflower-ce-sha512",
		.cra_priority		= 300,
...
static int sf_ce_sha384_digest(struct ahash_request *req)
{
	return sf_ce_sha384_init(req) ?: sf_ce_sha512_finup(req);
}

struct ahash_alg sf_ce_sha384 __read_mostly = {
	.init	= sf_ce_sha384_init,
	.update	= sf_ce_sha512_update,
	.final	= sf_ce_sha512_final,
	.finup	= sf_ce_sha512_finup,
	.digest	= sf_ce_sha384_digest,
	.export = sf_ce_sha512_export,
	.import = sf_ce_sha512_import,
	...
	.halg.base	=	{
		.cra_name		= "sha384",
		.cra_driver_name	= "siflower-ce-sha384",
		.cra_priority		= 300,
