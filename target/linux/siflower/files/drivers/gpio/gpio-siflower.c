...
#define to_sf_gpio(x)	container_of(x, struct sf_gpio, gc)

static u32 sf_gpio_rd(struct sf_gpio *priv, unsigned long reg)
{
	return readl_relaxed(priv->base + reg);
}

static void sf_gpio_wr(struct sf_gpio *priv, unsigned long reg,
		       u32 val)
{
